...
def get_public_toilet_by_name(db: Session, name: str):
    return db.query(models.PublicToilet).filter(models.PublicToilet.name == name).first()
...
def create_public_toilet(db: Session, public_toilet: schemas.PublicToilet, user: User):
    db_public_toilet = models.PublicToilet(name=public_toilet.name, lat=public_toilet.lat, lng=public_toilet.lng, user=user)
